...
class Home extends Component {
  ...
  filter = job => {
    ...
    if (
      selectedExperience.toString() != "--Any--" &&
  ...
  experienceFilter = job => {
    ...
    if (
      ...
      let str = job.experience
        .trim()
        .substring(0, job.experience.trim().lastIndexOf(" "));
      let firstNumber, secondNumber;
      if (str.includes("to")) {
        firstNumber = +str.slice(0, str.indexOf("to")).trim();
        secondNumber = +str.slice(str.indexOf("o") + 1, str.length);
        if (
          firstNumber <= selectedExperience &&
          selectedExperience <= secondNumber
        ) {
          if (keywords != "") {
  ...
  locationFilter = job => {
    ...
    if (
      job.location.toLowerCase().indexOf(selectedLocation.toLowerCase()) > -1
    ) {
      if (keywords != "") {
        if (this.matchKeyword(job)) {
          return true;
  ...
  matchKeyword = job => {
    ...
    if (
      job.title.toLowerCase().indexOf(keywords.toLowerCase()) > -1 ||
          ...
          <div className="middle_filter_area">
            ...
            <div className="sort_button">
              <div>
                ...
                <button
                  className="btn"
                  onClick={() => this.handleSort("sortByLocation")}
                >
                  Location
                </button>
                <button
                  className="btn"
                  onClick={() => this.handleSort("sortByExperience")}
                >
                  Experience
                </button>
              </div>
